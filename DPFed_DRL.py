...
class Actor(nn.Module):
	...
	def forward(self, state,wei):
		a = F.relu(self.bn1(self.l1(state)))
		a = F.relu(self.bn2(self.l2(a)))
		a = self.l3(a)
		ll = int(np.sqrt(self.action_dim))
		for j in range(len(a)):
			a[j] = a[j] * wei[j%ll]
		a = a.view(a.size(0), int(np.sqrt(self.action_dim)), -1)
		a = self.max_action * F.softmax(a,2)
		a = a.view(a.size(0), self.action_dim)
		return a
...
class TD3(object):
	...
	def select_action(self, state,wei):
		state = torch.FloatTensor(state.reshape(1, -1)).to(device)
		self.actor.eval()
		ans = self.actor(state,wei).cpu().data.numpy().flatten()
		self.actor.train()
		return ans


	def train(self, replay_buffer, wei,batch_size=256):
		self.total_it += 1
		...
		with torch.no_grad():
			...
			next_action = (
				self.actor_target(next_state,wei) + noise
		...
		if self.total_it % self.policy_freq == 0:
			...
			# Compute actor losse
			actor_loss = -self.critic.Q1(state, self.actor(state,wei)).mean()
